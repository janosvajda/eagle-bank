...
describe("UsersService", () => {
  ...
  it("returns 404 before checking ownership when the user is missing", async () => {
    ...
    await expect(service.getAuthorized("usr-missing", "usr-owner")).rejects.toMatchObject({
      statusCode: 404
  ...
  it("returns 403 for an existing user owned by someone else", async () => {
    ...
    await expect(service.getAuthorized(user.id, "usr-other")).rejects.toMatchObject({
      statusCode: 403
    } satisfies Partial<AppError>);
  });
  ...
  it("updates one field without overwriting omitted fields", async () => {
    ...
    expect(repository.update).toHaveBeenCalledWith(user.id, { name: "Updated" });
  });
